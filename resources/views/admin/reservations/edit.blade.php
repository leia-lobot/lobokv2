    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reservations.update", [$reservation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text" name="start_time" id="start_time" value="{{ old('start_time', $reservation->start_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('stop_time') ? 'is-invalid' : '' }}" type="text" name="stop_time" id="stop_time" value="{{ old('stop_time', $reservation->stop_time) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('resource') ? 'is-invalid' : '' }}" name="resource_id" id="resource_id" required>
                    @foreach($resources as $id => $resource)
                        <option value="{{ $id }}" {{ ($reservation->resource ? $reservation->resource->id : old('resource_id')) == $id ? 'selected' : '' }}>{{ $resource }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $company)
                        <option value="{{ $id }}" {{ ($reservation->company ? $reservation->company->id : old('company_id')) == $id ? 'selected' : '' }}>{{ $company }}</option>
                    @endforeach
                </select>
